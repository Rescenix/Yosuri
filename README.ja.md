...
<p align="center">
  <img src="./assets/preview.gif" alt="Yosuri 動作デモ" width="100%">
</p>
...
| **🧲 無料モデルプール + 集約API** | 7社の無料プロバイダ・18モデルを1つの OpenAI & Anthropic 互換エンドポイントに集約：30分ごとの死活監視で0-4段階評価、毎日リスト再取得で削除されたソースを退役、レート制限はサーキットブレーカーで回避、LRU で直近利用可能なモデルを優先。Claude Code / Cursor / Codex に Base URL と Key を1つ入れるだけで、`auto` が最適なソースへ自動ルーティング |
| **♾️ 無制限同時ワークフロー** | 各セッションがそれぞれの goroutine 上で独立したワークフローを実行：リアルタイムストリームもメッセージバッファも個別。スレッドプールなし、キューなし、人為的な上限なし。10個のセッションを同時に動かし、自由に切り替えても、各ストリームは自分のセッションにだけ書き込む |
...
| **📄 Office ファイル納品** | Agent が完成した Word / PowerPoint / Excel / PDF / CSV / HTML / Markdown ファイルをそのまま作り、納品カードで提供——プレビューとダウンロード付き、コードではなく完成物 |
...
👉 **[https://yosuri.com/](https://yosuri.com/)** 👈 最新リリースを最速でダウンロード。
...
- 🐛 バグ / 提案 → [GitHub Issues](https://github.com/Rescenix/Yosuri/issues)
- Windows リリースは CI でビルドされ SignPath で署名されます（[ポリシー](./docs/CODE_SIGNING_POLICY.md)）
- コアコード：[AGPL-3.0 License](./LICENSE)
