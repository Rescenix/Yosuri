...
**本地运行，无限 Token，一句话生成能一起开黑的可玩 HTML**
...
**全球首发互动交互 Agent** —— 跟 Aurora 说一句「做个能联机的贪吃蛇」，它当场写出 HTML 小游戏，预览窗口里你们直接开黑对战。鼠标、键盘经 CDP 双向打进同一个 headless Chromium，你能在面板里亲手玩 Agent 造的网页游戏，而不只是看截图。

### 一眼看懂 ResceneAgent

- **聊天框就是游戏厅** —— 一句话生成可玩的 HTML 小游戏（贪吃蛇 / 飞机大战 / 联机对战），预览窗口不是静态截图，而是真实可交互的 Chromium：Aurora 改完代码，你直接用鼠标点击、键盘操作去开黑，双向交互由后端 CDP 中继闭环。
...
- **生图只是顺手** —— 需要游戏贴图/立绘时，Agent 直接调用文生图工具，结果作为图片工件嵌入消息流，继续用于它正在写的页面，不跳出工作区搬运素材。
...
- **模型列表按提供方分组** —— 聊天窗的模型下拉按后端目录的 `vendor`（提供方）折叠分组，组内模型带选中 ✓ 标记，与设置面板「按厂商折叠」同源；只展示用户在设置里选为可用的模型，自定义配置归入「其他」。
