...
//   2) 长期记忆段（swiftnet.UnconditionalInject，归 memory 桶）
// 任一段为空则不下发，前端只渲染收到的段。
func HandleMemoryInject(c *gin.Context) {
	...
	out := []injectSeg{
		...
		{Key: "memory", Title: "长期记忆（身份常驻 / 工作态）", Raw: swiftnet.Default().UnconditionalInject(), Enabled: strings.TrimSpace(swiftnet.Default().UnconditionalInject()) != ""},
	}
