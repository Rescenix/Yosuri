...
func callNativeWebSearch(parent context.Context, argsJSON string) (nativeToolResult, error) {
	key := strings.TrimSpace(os.Getenv("BING_SEARCH_API_KEY"))
	if key == "" {
		return nativeToolResult{}, fmt.Errorf("联网搜索未启用：未配置 BING_SEARCH_API_KEY；已知 URL 请使用 web_fetch")
	}
	var args struct {
	...
	endpoint := strings.TrimSpace(os.Getenv("BING_SEARCH_ENDPOINT"))
	if endpoint == "" {
		endpoint = "https://api.bing.microsoft.com/v7.0/search"
	}
	u, err := url.Parse(endpoint)
	if err != nil || (u.Scheme != "http" && u.Scheme != "https") || u.Host == "" {
		return nativeToolResult{}, fmt.Errorf("BING_SEARCH_ENDPOINT 不是有效的 http(s) 地址")
	}
	query := u.Query()
	query.Set("q", args.Query)
	query.Set("count", fmt.Sprintf("%d", args.Count))
	query.Set("mkt", "zh-CN")
	query.Set("setLang", "zh")
	...
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		return nativeToolResult{}, fmt.Errorf("搜索请求失败: %w", err)
	}
	defer resp.Body.Close()
	body, err := io.ReadAll(io.LimitReader(resp.Body, 2*1024*1024))
	if err != nil {
		return nativeToolResult{}, fmt.Errorf("搜索响应读取失败: %w", err)
	}
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nativeToolResult{}, fmt.Errorf("搜索请求返回 HTTP %d: %s", resp.StatusCode, truncateChars(strings.TrimSpace(string(body)), 1000))
	}
	var payload struct {
		WebPages struct {
			Value []struct {
	...
	if err := json.Unmarshal(body, &payload); err != nil {
		return nativeToolResult{}, fmt.Errorf("搜索响应解析失败: %w", err)
	}
	if len(payload.WebPages.Value) == 0 {
		return nativeToolResult{Text: "未找到相关结果。"}, nil
	}
	lines := make([]string, 0, len(payload.WebPages.Value))
	for i, item := range payload.WebPages.Value {
		title := strings.TrimSpace(item.Name)
		if title == "" {
			title = "无标题"
		}
		lines = append(lines, fmt.Sprintf("%d. %s\n   URL: %s\n   摘要: %s",
			i+1, title, strings.TrimSpace(item.URL), strings.TrimSpace(item.Snippet)))
	}
