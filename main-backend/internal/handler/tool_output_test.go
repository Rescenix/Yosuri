...
func TestCompactToolOutputShortPassthrough(t *testing.T) {
	short := "print('我的世界')"
	if got := compactToolOutput("wf", "call_1", short); got != short {
...
func TestCompactToolOutputKeepsHeadAndTail(t *testing.T) {
	...
	out := compactToolOutput("wf_1", "call_9", head+body+tail)
...
func TestCompactToolOutputSpillsToReadableFile(t *testing.T) {
	...
	out := compactToolOutput("wf_2", "call_1", full)
	...
	// 收尾清理
	cleanupToolOutputSpills("wf_2")
	if entries, _ := os.ReadDir(spill); len(entries) != 0 {
		t.Error("任务收尾后溢出文件没清掉")
	}
}
...
func TestCompactToolOutputNoBrokenUTF8(t *testing.T) {
	...
	out := compactToolOutput("wf_3", "call_1", strings.Repeat("汉", 8000))
	for _, r := range out {
		if r == '�' {
			t.Fatal("截断切碎了多字节字符")
		}
