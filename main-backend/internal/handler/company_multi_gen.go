...
func deliveryMultiProject(project, brief, upstream string) companyMultiFiles {
	...
	css, err := deliveryLLMContent("设计师", brief, "样式",
		"写该产品的 styles.css，设计规格（硬要求）：①:root 定义 CSS 变量（主色+辅色+中性灰阶 3 档以上），配色和谐有品牌感，禁止默认浏览器蓝紫 #3B82F6/#6366f1，禁止纯黑纯白大面积对撞；②卡片圆角 12-16px、多层细腻阴影、hover 有 translateY 过渡 ≥.18s；③中文排版 font-family 带 PingFang SC/Microsoft YaHei、行高≥1.6、数字 tabular-nums；④响应式 @media 至少 640px/960px 两档断点；⑤表单控件有 focus 态样式；⑥主容器用 min-height:100vh 的 flex 布局，统计卡片区/说明区撑满视口，禁止下半页塌陷成空白；⑦合计/统计区用带背景色的卡片（不是裸文本），正负值有绿/红色区分。若上游产物含【视觉参考规格】，主色/辅色/布局气质必须贴合该既定方向。只输出 CSS 代码本体，不要 markdown 围栏",
		upstream)
	...
	appJS, err := deliveryLLMContent("前端工程师", brief, "应用逻辑",
		"数据接口契约（必须严格照此调用，data.js 已固定实现）：window.AppData.list() 返回对象数组；window.AppData.add(obj) 新增并返回带 id 的对象（自动带 createdAt 时间戳），obj 字段自定但必须一次传全；window.AppData.update(id,patch) 局部更新；window.AppData.remove(id) 删除；window.AppData.byDay(field) 按日期聚合返回 [{day:'YYYY-MM-DD',count:N}]（field 缺省按 createdAt，可传自定义日期字段名），打卡/习惯/统计类产品直接用它做每日趋势，不要自己另写日期分组逻辑。页面上所有 DOM（表单、列表容器、合计区、空状态）由 app.js 自己创建并挂载到 document.getElementById('app') 下，严禁假设 index.html 里已有其它元素。交互质感（硬要求）：①操作后列表有可见反馈（新条目高亮淡入、删除有过渡）；②空状态要精心设计（图标+引导文案+主操作按钮，不是一行灰字）；③统计数字变化用 CSS 过渡而非瞬间跳变；④列表为空/加载中/出错三种状态都要处理；⑤收支/数值类产品：合计区做成统计卡片（收入/支出/净额三卡），净额为负标红；⑥页面底部要有内容（统计卡/使用说明/分类汇总），禁止下半页空白。只输出 JS 代码本体，不要 markdown 围栏",
		upstream)
