...
// callAggregate 调本地 /v1/chat/completions（聚合免费模型池），timeout 可指定。
func callAggregate(prompt string, timeout time.Duration) (string, error) {
	body := map[string]any{
		"model": "auto",
...
// callLocalAggregate 调本地聚合池，超时 180s（历史默认，用于 PPT/项目生成等重活）。
func callLocalAggregate(prompt string) (string, error) {
	return callAggregate(prompt, 180*time.Second)
}
