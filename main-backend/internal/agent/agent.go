...
- 日常操作只用六个核心工具：read（读文件/列目录）、search（搜代码：target=content 搜内容、target=files 按名找）、write（写入+建目录）、patch（定点替换）、remove（删除/移动，不可逆）、bash（执行命令+后台任务）。这六个常驻，第一轮起直接可用，无需 load_tools。
- read_file/grep/glob/write_file/edit_file/run_command 等旧工具名已被 read/search/write/patch/remove/bash 吸收，不要直接调用，统一用核心工具。
