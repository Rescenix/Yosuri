...
// 项目根目录（与 handler.go 中的 projectRoot 保持一致）
const projectRootDir = `C:\Pro2026\re0`
...
func InitCodebaseIndex() error {
    ...
    indexOnce.Do(func() {
        ...
        if err != nil {
            initErr = fmt.Errorf("创建代码索引失败: %w", err)
            return
        }
        ...
        if err != nil {
            initErr = fmt.Errorf("扫描项目目录失败: %w", err)
            return
        }
        codeIndex = idx
...
func UpdateCodeIndex(filePath string) error {
    ...
    if !filepath.IsAbs(absPath) {
        absPath = filepath.Join(projectRootDir, absPath)
    }
    if _, err := os.Stat(absPath); os.IsNotExist(err) {
        // 文件不存在，从索引中移除
        relPath, _ := filepath.Rel(projectRootDir, absPath)
        if relPath != "" {
    ...
    relPath, err := filepath.Rel(projectRootDir, absPath)
    if err != nil {
